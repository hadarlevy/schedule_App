...
public class EditAdminDetailsActivity extends AppCompatActivity {
            ...
            db.collection("Manager").document(user.getUid()).get()
                    .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                        ...
                        public void onSuccess(DocumentSnapshot documentSnapshot) {
                            if (documentSnapshot.exists()) {
                                ...
                                Toast.makeText(EditAdminDetailsActivity.this, "User details not found", Toast.LENGTH_SHORT).show();
                                finish();
                            }
                        }
                    })
                    .addOnFailureListener(new OnFailureListener() {
                        ...
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(EditAdminDetailsActivity.this, "Failed to fetch user details: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    });
        } else {
            // Handle case where user is null
            Toast.makeText(EditAdminDetailsActivity.this, "User not authenticated", Toast.LENGTH_SHORT).show();
            finish();
        }
    ...
    private void saveUserDetails() {
        ...
        if (validateInput(firstName, lastName, phoneNumber)) {
            if (user != null) {
                DocumentReference userRef = db.collection("Manager").document(user.getUid());
                userRef.update("First name", firstName, "Last name", lastName, "Phone", phoneNumber)
                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                            @Override
                            public void onSuccess(Void aVoid) {
                                Toast.makeText(EditAdminDetailsActivity.this, "Details updated successfully", Toast.LENGTH_SHORT).show();
                                setResult(RESULT_OK);
                                finish();
                            }
                        })
                        .addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Toast.makeText(EditAdminDetailsActivity.this, "Error updating details: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                            }
                        });
            } else {
                Toast.makeText(EditAdminDetailsActivity.this, "User not authenticated", Toast.LENGTH_SHORT).show();
